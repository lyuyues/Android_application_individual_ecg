...
public class MADetect {
	...
	//return the new ecg data
	public List<Double> getNewECG() {
		return new_ecg;
	}
	
	private boolean read_ECG_Data(String file_path, List<Double> ecg_data) {
		File file = new File(file_path);
        byte[] buffer = new byte[2];
        try {
			FileInputStream in = new FileInputStream(file);
			while (in.read(buffer) != -1) {
				ecg_data.add(((buffer[0] << 8) & 0xFFFF)*1.0+ (buffer[1] & 0xFF));
			}
			int sum = 0;
	...
	private boolean Detect() {
		...
		for (int k = 0; k < minSize; ++k){
			...
			if (temp>0){
				MAresult.add(1); //motion artifacts occur
			}else {
	...
	public boolean run() {
		if (!read_ECG_Data(path, ecg_data)) {
			return false;
		}
